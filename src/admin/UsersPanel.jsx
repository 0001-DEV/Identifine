...
export default function UsersPanel() {
  ...
  const handleAddUser = (e) => {
    ...
    const updated = { ...settings, users: [...users, { id: String(Date.now()), name: newUser.name, email: newUser.email, role: newUser.role, username: newUser.username || newUser.name.toLowerCase().replace(/\s+/g, '.') }] };
    setSettings(updated);
  ...
  return (
    <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <h1 style={{ fontSize: 23, fontWeight: 400, margin: 0, color: '#1d2327' }}>Users</h1>
        <button
      ...
      {showAdd && (
        <div style={{ background: '#fff', border: '1px solid #c3c4c7', padding: 20, marginBottom: 20, boxShadow: '0 1px 1px rgba(0,0,0,.04)' }}>
          <h2 style={{ fontSize: 16, fontWeight: 600, margin: '0 0 16px', color: '#1d2327' }}>Add New User</h2>
          <form onSubmit={handleAddUser}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#3c434a', marginBottom: 4 }}>Username <span style={{ color: '#d63638' }}>*</span></label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#3c434a', marginBottom: 4 }}>Full Name <span style={{ color: '#d63638' }}>*</span></label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#3c434a', marginBottom: 4 }}>Email <span style={{ color: '#d63638' }}>*</span></label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#3c434a', marginBottom: 4 }}>Role</label>
                <select value={newUser.role} onChange={e => setNewUser(n => ({ ...n, role: e.target.value }))} style={{ ...inputStyle, cursor: 'pointer' }}>
                  {Object.values(ROLES).map(r => <option key={r.id} value={r.id}>{r.label.replace(/^\S+\s/, '')}</option>)}
                </select>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button type="button" onClick={() => setShowAdd(false)} style={{ background: '#f0f0f1', border: '1px solid #8c8f94', borderRadius: 3, padding: '6px 10px', fontSize: 13, cursor: 'pointer', color: '#3c434a' }}>Cancel</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div style={{ marginBottom: 8, fontSize: 13 }}>
        <span style={{ fontWeight: 700, color: '#1d2327', fontSize: 13 }}>All ({users.length})</span>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        ...
        <button style={{ background: '#f0f0f1', border: '1px solid #8c8f94', borderRadius: 3, padding: '4px 10px', fontSize: 13, cursor: 'pointer', color: '#3c434a' }}>Change</button>
        ...
        <button style={{ background: '#f0f0f1', border: '1px solid #8c8f94', borderRadius: 3, padding: '4px 10px', fontSize: 13, cursor: 'pointer', color: '#3c434a' }}>Search Users</button>
      </div>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', background: '#fff', border: '1px solid #c3c4c7', boxShadow: '0 1px 1px rgba(0,0,0,.04)' }}>
        ...
        <tbody>
          {filtered.length === 0 && (
            <tr><td colSpan={6} style={{ padding: 20, textAlign: 'center', fontSize: 13, color: '#646970' }}>No users found.</td></tr>
          )}
          {filtered.map((user, i) => {
            const roleInfo = ROLES[user.role] || ROLES.AUTHOR;
            return (
              <tr key={user.id} style={{ background: i % 2 === 0 ? '#fff' : '#f9f9f9' }}>
                ...
                <td style={tdStyle}>
                  <select
                    ...
                    style={{ padding: '3px 6px', fontSize: 12, border: '1px solid #8c8f94', borderRadius: 3, outline: 'none', background: `${roleInfo.color}10`, color: roleInfo.color, fontWeight: 600, cursor: 'pointer' }}
                  >
                    {Object.values(ROLES).map(r => <option key={r.id} value={r.id}>{r.label.replace(/^\S+\s/, '')}</option>)}
                  </select>
                </td>
      ...
      <div style={{ fontSize: 12, color: '#646970', marginTop: 8 }}>{filtered.length} user{filtered.length !== 1 ? 's' : ''}</div>
    </div>
  );
}
