...
// Exactly matching WP Rank Math Role Manager - checkboxes per capability per role
const CAPABILITIES = [
  ...
  return (
    <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
        ...
        <h1 style={{ fontSize: 23, fontWeight: 400, margin: 0, color: '#1d2327' }}>Role Manager</h1>
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #c3c4c7', padding: 16, marginBottom: 20, boxShadow: '0 1px 1px rgba(0,0,0,.04)' }}>
        <h2 style={{ fontSize: 14, fontWeight: 600, color: '#1d2327', margin: '0 0 12px' }}>Active User Role (Simulate)</h2>
        <p style={{ fontSize: 13, color: '#646970', margin: '0 0 12px' }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: activeRole === roleId ? `2px solid ${r.color}` : '1px solid #c3c4c7',
                  background: activeRole === roleId ? `${r.color}15` : '#f9f9f9',
                  color: activeRole === roleId ? r.color : '#3c434a',
                }}
              >
                {r.label}
              </button>
        ...
        <div style={{ marginTop: 10, fontSize: 12, color: '#646970' }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #c3c4c7', boxShadow: '0 1px 1px rgba(0,0,0,.04)', marginBottom: 20 }}>
        <div style={{ padding: '10px 16px', borderBottom: '1px solid #c3c4c7', background: '#f9f9f9' }}>
          <h2 style={{ fontSize: 14, fontWeight: 600, margin: 0, color: '#1d2327' }}>Capabilities Matrix</h2>
          <p style={{ fontSize: 12, color: '#646970', margin: '4px 0 0' }}>Check which roles can access each SEO capability.</p>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 600 }}>
            <thead>
              <tr>
                <th style={{ padding: '10px 16px', borderBottom: '1px solid #c3c4c7', fontSize: 13, fontWeight: 600, color: '#1d2327', textAlign: 'left', background: '#f9f9f9', width: '40%' }}>Capability</th>
                {roleOrder.map(roleId => (
                  <th key={roleId} style={{ padding: '10px 16px', borderBottom: '1px solid #c3c4c7', fontSize: 13, fontWeight: 600, color: ROLES[roleId]?.color || '#1d2327', textAlign: 'center', background: '#f9f9f9' }}>
                    {ROLES[roleId]?.label}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {CAPABILITIES.map((cap, i) => (
                <tr key={cap.id} style={{ background: i % 2 === 0 ? '#fff' : '#f9f9f9' }}>
                  <td style={{ padding: '12px 16px', borderBottom: '1px solid #f0f0f1', verticalAlign: 'top' }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#3c434a' }}>{cap.label}</div>
                    <div style={{ fontSize: 11, color: '#646970', marginTop: 2, lineHeight: 1.4 }}>{cap.desc}</div>
                  </td>
                  {roleOrder.map(roleId => (
                    <td key={roleId} style={{ padding: '12px 16px', borderBottom: '1px solid #f0f0f1', textAlign: 'center' }}>
                      <input
                        type="checkbox"
                        checked={!!(permissions[roleId] || {})[cap.id]}
                        onChange={() => togglePerm(roleId, cap.id)}
                        disabled={roleId === 'ADMIN'} // Admin always has all permissions
